...
class LaneKeepingEnv(AbstractEnv):
    ...
    专注于车道保持控制任务
        ...
        连续控制：使用连续转向动作而非离散选择
        物理真实：基于BicycleVehicle的精确车辆动力学
        道路复杂：正弦波弯道+直线道的组合设计
        观察丰富：包含状态、导数、参考的多维观察
        奖励精密：基于横向偏差的连续奖励函数
        噪声仿真：模拟真实传感器的不确定性
    """
        ...
        config.update(
            {
                ...
                "simulation_frequency": 10,   # 每秒进行的物理仿真步数
                "policy_frequency": 10,       # 每秒进行的策略更新步数
                "state_noise": 0.05,          # 给状态观测加的均匀分布噪声，[-0.05, 0.05]
                "derivative_noise": 0.05,     # 给导数观测加的均匀分布噪声，[-0.05, 0.05]
                "screen_width": 600,
                "screen_height": 250,
                "scaling": 7,
