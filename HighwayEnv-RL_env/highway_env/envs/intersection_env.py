...
class IntersectionEnv(AbstractEnv):
        ...
        config.update(
            {
                ...
                "spawn_probability": 0.6,   # 每个时间步有60%概率在交叉路口生成新车辆
                "screen_width": 600,     
                "screen_height": 600,
                "centering_position": [0.5, 0.6],   # 相机中心位置
                "scaling": 5.5 * 1.3,               
                "collision_reward": -5,
                "high_speed_reward": 1,
                "arrived_reward": 1,
    ...
    def _rewards(self, action: int) -> dict[str, float]:
        ...
        for name in agents_rewards[0].keys():
            for agent_rewards in agents_rewards:
                total+=agent_rewards[name]
            result[name]=total/len(agents_rewards)
            total=0
        return result
        """
        return {   
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
        ...
        # 第3步：到达奖励覆盖 (防止碰撞惩罚过严重 导致 即使到达了也是负奖励)
    ...
    def _agent_rewards(self, action: int, vehicle: Vehicle) -> dict[str, float]:
        ...
        return {
            "collision_reward": vehicle.crashed,  # 1 if crashed, 0 otherwise
            "high_speed_reward": np.clip(scaled_speed, 0, 1),  # 0 to 1
            "arrived_reward": self.has_arrived(vehicle),  # 1 if arrived, 0 otherwise
            "on_road_reward": vehicle.on_road,   # 1 if on road, 0 otherwise
        }
