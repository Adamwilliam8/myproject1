...
def call_llm(model_name: str, config: dict, messages):
    ...
    with open(os.path.join(output_dir, 'reward_modify_output.txt'), 'w') as f:
        print(reward_modify_output, file=f)
    ...
    with open(os.path.join(output_dir, 'reward_modify_output.txt'), 'r') as file:
